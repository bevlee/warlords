...
<div class="flex items-center gap-2">
  ...
  <div class="turnbar-scroll flex min-w-0 items-center gap-1.5 overflow-x-scroll overflow-y-hidden py-2">
    {#each entries as unit, i (`${unit.id}-${i}`)}
      <button
        type="button"
        class="portrait relative h-16 w-14 shrink-0 overflow-hidden rounded-sm border-2 transition-transform
      ...
      >
        ...
        <span
          class="absolute bottom-0 right-0 bg-black/70 px-1 font-mono text-xs font-bold leading-tight text-amber-300"
        >
          {unit.count}
        </span>
      </button>
      {#if i === 0}
        <div class="h-14 w-px shrink-0 bg-slate-600" aria-hidden="true"></div>
      {/if}
    {/each}
  </div>
</div>

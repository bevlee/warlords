<script lang="ts">
  ...
  import { entryHref, factionSkillId, unitEntries } from '$lib/compendium/entries';
  ...
  let { entry, discovery }: Props = $props();
  ...
  <div class="mt-4 border-t border-slate-700 pt-3">
    ...
    <div class="flex flex-col gap-2">
      {#each FACTION_SKILL_DEFS[entry.faction] as skill (skill.id)}
        <a href={entryHref('factionSkill', factionSkillId(entry.faction, skill.id))} class="block hover:underline">
...
{:else if entry.kind === 'ability'}
  ...
  {#if entry.teachable}
    <p class="mt-2 text-sm text-violet-300">
      ...
      <a href={entryHref('unitSkill', entry.id)} class="font-semibold underline">gauntlet skill draft</a>.
    </p>
  {/if}
  ...
  <div class="mt-4 border-t border-slate-700 pt-3">
    ...
    <div class="flex flex-col gap-1">
      {#each entry.units as slug (slug)}
        {@const unit = byId.get(slug)}
        {#if unit}
          <a href={entryHref('unit', slug)} class="flex items-center gap-2 rounded px-1.5 py-1 hover:bg-slate-700/60">
            <Sprite name={unit.name} class="h-9 w-8 shrink-0" />
            <span class="flex-1 truncate text-sm font-semibold {TIER_STYLE[unit.tier].text}">{unit.name}</span>
...
{:else if entry.kind === 'factionSkill'}
  ...
  <p class="mt-1 text-[11px] font-bold uppercase tracking-wider text-slate-400">
    <a href={entryHref('faction', entry.faction)} class="hover:text-amber-300">{FACTION_INFO[entry.faction].name}</a>
...
{:else if entry.kind === 'unitSkill'}
  ...
  <p class="mt-3 border-t border-slate-700 pt-3 text-sm text-slate-400">
    ...
    <a href={entryHref('ability', entry.ability)} class="font-semibold text-amber-300 hover:underline">ability</a>
